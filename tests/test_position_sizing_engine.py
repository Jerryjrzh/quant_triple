...
class TestDynamicPositionSizingEngine:
    ...
    @pytest.mark.asyncio
    async def test_calculate_position_size_kelly(
        self, sizing_engine, sample_price_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_calculate_position_size_var_based(
        self, sizing_engine, sample_price_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_calculate_position_size_with_risk_metrics(
        self, sizing_engine, sample_price_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_optimize_portfolio_risk_budget_risk_parity(
        self, sizing_engine, sample_portfolio_assets
    ):
    ...
    @pytest.mark.asyncio
    async def test_optimize_portfolio_risk_budget_inverse_volatility(
        self, sizing_engine, sample_portfolio_assets
    ):
    ...
    async def test_analyze_concentration_risk(self, sizing_engine):
        ...
        result_concentrated = await sizing_engine.analyze_concentration_risk(
            concentrated_weights
        )

        assert result_concentrated.concentration_level in [
    ...
    async def test_analyze_concentration_risk_with_sectors(self, sizing_engine):
        ...
        assert any(
            "sector" in warning.lower() or "technology" in warning.lower()
    ...
    @pytest.mark.asyncio
    async def test_generate_portfolio_recommendations_with_transaction_costs(
        self, sizing_engine
    ):
    ...
    async def test_insufficient_data_handling(self, sizing_engine):
        ...
        with pytest.raises(ValueError, match="Insufficient data"):
            await sizing_engine.calculate_position_size(
                symbol="TEST", price_data=minimal_data, portfolio_value=1000000
            )
...
class TestEdgeCases:
    ...
    async def test_single_asset_portfolio(self, sizing_engine):
        ...
        result = await sizing_engine.optimize_portfolio_risk_budget(
            assets=single_asset, portfolio_value=1000000
        )
