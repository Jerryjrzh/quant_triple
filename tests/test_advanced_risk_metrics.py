...
class TestAdvancedRiskMetricsCalculator:
    ...
    def mock_spring_festival_engine(self):
        ...
        mock_engine = Mock()
        mock_engine.analyze_seasonal_patterns = AsyncMock(
            return_value={"seasonal_volatility": 0.25, "current_risk_level": "moderate"}
        )
        return mock_engine

    @pytest.mark.asyncio
    async def test_calculate_advanced_metrics(
        self,
        calculator,
        sample_price_data,
        ...
        if result.sharpe_confidence_interval:
            ...
            assert (
                result.sharpe_confidence_interval[0]
                <= result.sharpe_confidence_interval[1]
            )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_sharpe_ratio_calculation(
        self, calculator, sample_price_data
    ):
        """Test enhanced Sharpe ratio with confidence intervals."""

        returns = calculator._calculate_returns(sample_price_data)
        sharpe, confidence_interval = await calculator._calculate_enhanced_sharpe_ratio(
            returns
        )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_sortino_ratio_calculation(
        self, calculator, sample_price_data
    ):
        """Test enhanced Sortino ratio with confidence intervals."""

        returns = calculator._calculate_returns(sample_price_data)
        sortino, confidence_interval = (
            await calculator._calculate_enhanced_sortino_ratio(returns)
        )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_calmar_ratio_calculation(
        self, calculator, sample_price_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_market_risk_metrics_calculation(
        self, calculator, sample_price_data, sample_benchmark_data
    ):
        """Test comprehensive market risk metrics."""

        returns = calculator._calculate_returns(sample_price_data)
        beta, beta_ci, alpha, tracking_error, info_ratio = (
            await calculator._calculate_market_risk_metrics(
    ...
    @pytest.mark.asyncio
    async def test_liquidity_risk_metrics_calculation(
        self, calculator, sample_price_data, sample_volume_data
    ):
        ...
        assert result["spread_proxy"] is None or isinstance(
            result["spread_proxy"], float
        )
        assert result["impact_score"] is None or isinstance(
    ...
    async def test_price_based_liquidity_metrics(self, calculator, sample_price_data):
        ...
        result = await calculator._calculate_price_based_liquidity_metrics(
            sample_price_data
        )
    ...
    @pytest.mark.asyncio
    async def test_seasonal_risk_metrics_calculation(
        self, calculator, sample_price_data, mock_spring_festival_engine
    ):
...
class TestComprehensiveRiskProfile:
    ...
    def sample_data_set(self):
        ...
        benchmark_data = pd.DataFrame({"date": dates, "close": bench_prices})

        # Volume data
        volume_data = pd.DataFrame(
            {"date": dates, "volume": np.random.lognormal(10, 1, len(dates))}
        )
    ...
    def test_overall_risk_score_calculation(self):
        ...
        basic_metrics.volatility_results = {
            "historical": Mock(annualized_volatility=0.25)
        }
    ...
    def test_risk_level_determination(self):
        ...
        with patch(
            "stock_analysis_system.analysis.advanced_risk_metrics._calculate_overall_risk_score"
        ) as mock_score:
    ...
    def test_key_risk_factors_identification(self):
        ...
        basic_metrics.volatility_results = {
            "historical": Mock(annualized_volatility=0.5)
        }  # High volatility
