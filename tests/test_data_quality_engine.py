...
class TestCompletenessRule:
        ...
        data = pd.DataFrame(
            {
                "close_price": [
                    ...
                    12.0,
                    13.0,
                ]  # 20% missing, but threshold is 10%
            }
        )
    ...
    def test_no_missing_values(self):
        ...
        data = pd.DataFrame({"close_price": [10.0, 11.0, 12.0, 13.0]})
...
class TestConsistencyRule:
    ...
    def test_ohlc_consistency_violations(self):
        ...
        issues = rule.validate(data)
        high_issues = [
            i for i in issues if "high_price is not the highest" in i.description
        ]
...
class TestTimelinessRule:
    ...
    def test_stale_data(self):
        ...
        data = pd.DataFrame({"trade_date": [old_date, datetime.now()]})

        issues = rule.validate(data)
        stale_issues = [
            i for i in issues if i.issue_type == DataQualityIssueType.STALE_DATA
        ]
    ...
    def test_future_dates(self):
        ...
        data = pd.DataFrame({"trade_date": [datetime.now(), future_date]})
    ...
    def test_invalid_date_format(self):
        ...
        rule = TimelinessRule("trade_date")
        data = pd.DataFrame({"trade_date": ["invalid_date", "2024-01-01"]})

        issues = rule.validate(data)
        format_issues = [
            i for i in issues if i.issue_type == DataQualityIssueType.INVALID_FORMAT
        ]
...
class TestDuplicateRule:
    ...
    def test_missing_key_columns(self):
        ...
        data = pd.DataFrame({"stock_code": ["000001.SZ"], "close_price": [10.0]})
...
class TestMLAnomalyDetector:
    ...
    def test_fit_and_detect(self):
        ...
        assert any(
            idx >= 100 for idx in anomaly_indices
        )  # Outliers are at indices 100, 101

    def test_save_and_load_model(self):
        """Test saving and loading model."""
        data = pd.DataFrame(
            {"price": [100, 110, 90, 105], "volume": [1000, 1100, 900, 1050]}
        )
...
class TestEnhancedDataQualityEngine:
    ...
    def test_ml_anomaly_detection_integration(self, sample_stock_data):
        ...
        outlier_issues = [
            ...
            if i.issue_type == DataQualityIssueType.OUTLIER_DATA
        ]
    ...
    def test_model_save_load(self, sample_stock_data):
        ...
        with tempfile.NamedTemporaryFile(suffix=".joblib", delete=False) as tmp:
            try:
                ...
                report = new_engine.validate_data(
                    sample_stock_data, "loaded_model_test"
                )
