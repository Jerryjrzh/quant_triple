...
class TestInstitutionClassifier:
    ...
    def test_classify_unknown(self, classifier):
        ...
        unknown_names = ["Unknown Institution", "随机公司名称", "XYZ Corporation"]
...
class TestInstitutionalDataCollector:
        ...
        for stock_code in stock_codes:
            ...
            if len(activities) > 1:
                for i in range(1, len(activities)):
                    assert (
                        activities[i - 1].activity_date <= activities[i].activity_date
                    )
    ...
    def test_get_institution_activity_timeline(self, collector):
        ...
        assert (
            fund_activities[0].institution.institution_type
