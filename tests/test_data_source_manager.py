...
class TestRateLimiter:
    ...
    async def test_rate_limiter_basic(self):
        ...
        config = RateLimitConfig(requests_per_minute=2, burst_limit=1, cooldown_period=1)
    ...
    async def test_rate_limiter_burst_limit(self):
        ...
        config = RateLimitConfig(requests_per_minute=10, burst_limit=2, cooldown_period=1)
        limiter = RateLimiter(config)
...
class TestTushareDataSource:
    ...
    async def test_get_stock_data_success(self, mock_tushare_api):
        ...
        with patch('stock_analysis_system.data.data_source_manager.settings') as mock_settings:
            ...
            result = await source.get_stock_data('000001.SZ', date(2024, 1, 1), date(2024, 1, 1))
...
class TestAkshareDataSource:
    ...
    async def test_test_connection_failure(self):
        """Test connection test failure."""
        with patch('akshare.stock_zh_a_hist', side_effect=Exception("Connection failed")):
            source = AkshareDataSource()
            result = await source.test_connection()
...
class TestDataSourceManager:
    ...
    async def test_get_stock_data_primary_success(self, manager):
        ...
        with patch.object(manager.data_sources[DataSourceType.TUSHARE], 'get_stock_data', 
                         return_value=mock_data):
            result = await manager.get_stock_data('000001.SZ', date(2024, 1, 1), date(2024, 1, 1))
    ...
    async def test_get_stock_data_failover(self, manager):
        ...
        with patch.object(manager.data_sources[DataSourceType.TUSHARE], 'get_stock_data', 
            ...
            with patch.object(manager.data_sources[DataSourceType.AKSHARE], 'get_stock_data', 
                             return_value=mock_data):
                result = await manager.get_stock_data('000001.SZ', date(2024, 1, 1), date(2024, 1, 1))
        ...
        with patch.object(manager.data_sources[DataSourceType.TUSHARE], 'get_stock_data', 
            ...
            with patch.object(manager.data_sources[DataSourceType.AKSHARE], 'get_stock_data', 
                ...
                with pytest.raises(Exception):
                    await manager.get_stock_data('000001.SZ', date(2024, 1, 1), date(2024, 1, 1))
    
    @pytest.mark.asyncio
    async def test_health_check(self, manager):
        """Test health check functionality."""
        # Mock connection tests
        with patch.object(manager.data_sources[DataSourceType.TUSHARE], 'test_connection', 
                         return_value=True):
            with patch.object(manager.data_sources[DataSourceType.AKSHARE], 'test_connection', 
                             return_value=False):
                ...
                assert health_status[DataSourceType.TUSHARE].status == DataSourceStatus.HEALTHY
                assert health_status[DataSourceType.AKSHARE].status == DataSourceStatus.DEGRADED
