...
class RateLimiter:
    ...
    async def acquire(self):
        ...
        self.burst_requests = [
            req_time
            for req_time in self.burst_requests
...
class BaseDataSource(ABC):
    ...
    async def get_intraday_data(
        self, symbol: str, start_date: date, end_date: date, timeframe: str = "5min"
    ) -> pd.DataFrame:
    ...
    def _record_success(self, execution_time: float):
        ...
        self.health.avg_response_time = (self.health.avg_response_time * 0.9) + (
            execution_time * 0.1
        )
    ...
    def _update_reliability_score(self):
        ...
        total_requests = (
            self.health.failure_count + 100
        )  # Assume some successful requests
        ...
        if self.health.last_failure:
            hours_since_failure = (
                datetime.now() - self.health.last_failure
            ).seconds / 3600
...
class LocalDataSource(BaseDataSource):
    ...
    async def get_stock_data(
        self, symbol: str, start_date: date, end_date: date, timeframe: str = "daily"
    ) -> pd.DataFrame:
            ...
            try:
                ...
                if timeframe == "daily":
                    ...
                    data_file = os.path.join(
                        self.base_path, market, "lday", f"{market}{stock_code}.day"
                    )
                    ...
                    data_file = os.path.join(
                        self.base_path, market, "fzline", f"{market}{stock_code}.lc5"
                    )
                    ...
                    if not os.path.exists(data_file):
                        logger.warning(
                            f"Local 5min data file not found for resampling: {data_file}"
                        )
                        return pd.DataFrame()
                ...
                if timeframe == "daily":
                    ...
                    start_datetime = pd.Timestamp.combine(
                        start_date, pd.Timestamp.min.time()
                    )
                    end_datetime = pd.Timestamp.combine(
                        end_date, pd.Timestamp.max.time()
                    )
        ...
        try:
            with open(file_path, "rb") as f:
                while True:
                    ...
                    try:
                        # Unpack the data
                        date, open_p, high_p, low_p, close_p, amount, volume = (
                            struct.unpack(unpack_format, chunk[:unpack_size])
                        )
    ...
    def _resample_5min_data(
        ...
        try:
            ...
            resample_rules = {"15min": "15min", "30min": "30min", "60min": "60min"}
            ...
            # Resample the data
            resampled = (
                df_resampled.resample(resample_rules[timeframe])
                .agg(
                    {
                        "open": "first",
                        "high": "max",
        ...
        try:
            ...
            for market in ["sz", "sh"]:
                ...
                if os.path.exists(market_path):
                    ...
                    day_files = [
                        f for f in os.listdir(market_path) if f.endswith(".day")
                    ]
                    if day_files:
                        return True
...
class DataSourceManager:
    ...
    async def get_stock_list(self) -> pd.DataFrame:
        ...
        for source_type in self.fallback_order:
            ...
            try:
                ...
                if not data.empty:
                    logger.info(
                        f"Successfully fetched stock list from {source_type.value}"
                    )
                    return data

            except Exception as e:
                logger.error(
                    f"Failed to fetch stock list from {source_type.value}: {e}"
                )
                last_exception = e
                continue
    ...
    async def get_intraday_data(
        ...
        for source_type in self.fallback_order:
            ...
            try:
                logger.info(
                    f"Attempting to fetch intraday data from {source_type.value}"
                )
                data = await data_source.get_intraday_data(
                    symbol, start_date, end_date, timeframe
                )

                if not data.empty:
                    logger.info(
                        f"Successfully fetched intraday data from {source_type.value}"
                    )
                    return data
                else:
                    logger.warning(
                        f"No intraday data returned from {source_type.value}"
                    )

            except Exception as e:
                logger.error(
                    f"Failed to fetch intraday data from {source_type.value}: {e}"
                )
                last_exception = e
                continue
