...
class MLAnomalyDetector:
    ...
    def fit(self, data: pd.DataFrame, feature_columns: List[str]) -> None:
        ...
        self.is_fitted = True
        logger.info(
            f"ML anomaly detector fitted on {len(data)} samples with {len(feature_columns)} features"
        )
...
class EnhancedDataQualityEngine:
    ...
    def train_ml_detector(
        self, training_data: pd.DataFrame, feature_columns: List[str] = None
    ):
    ...
    def validate_data(
        ...
        if self.ml_detector.is_fitted:
            try:
                anomaly_indices, anomaly_scores = self.ml_detector.detect_anomalies(
                    data
                )
                if anomaly_indices:
                    all_issues.append(
                ...
                logger.info(
                    f"ML anomaly detection found {len(anomaly_indices)} potential issues"
                )
            except Exception as e:
        ...
        logger.info(
            f"Data quality validation completed. Overall score: {scores['overall']:.2f}"
        )
        return report

    def _calculate_quality_scores(
        self, data: pd.DataFrame, issues: List[DataQualityIssue]
    ) -> Dict[str, float]:
        """Calculate quality scores based on issues found."""
        total_rows = len(data)

        # Count issues by type
        issue_counts = {"missing": 0, "consistency": 0, "timeliness": 0, "accuracy": 0}

        for issue in issues:
            affected_ratio = (
                len(issue.affected_rows) / total_rows if total_rows > 0 else 0
            )
            severity_weight = {
            ...
            if issue.issue_type in [DataQualityIssueType.MISSING_DATA]:
                ...
                DataQualityIssueType.INCONSISTENT_DATA,
                DataQualityIssueType.BUSINESS_RULE_VIOLATION,
            ]:
                issue_counts["consistency"] += impact
            elif issue.issue_type in [DataQualityIssueType.STALE_DATA]:
                issue_counts["timeliness"] += impact
            elif issue.issue_type in [
                DataQualityIssueType.OUTLIER_DATA,
                DataQualityIssueType.DUPLICATE_DATA,
            ]:
    ...
    def _generate_recommendations(self, issues: List[DataQualityIssue]) -> List[str]:
        ...
        if any(issue.severity == DataQualitySeverity.CRITICAL for issue in issues):
            recommendations.append(
                "Address critical issues immediately before using data for analysis"
            )
    ...
    def clean_data(self, data: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
        ...
        for issue in report.issues:
            if issue.issue_type == DataQualityIssueType.DUPLICATE_DATA:
                # Remove duplicates
                if (
                    "stock_code" in cleaned_data.columns
                    and "trade_date" in cleaned_data.columns
                ):
                ...
                issue.issue_type == DataQualityIssueType.OUTLIER_DATA
                and issue.severity != DataQualitySeverity.CRITICAL
            ):
                # Remove extreme outliers (only if not critical)
                if issue.affected_rows:
                    cleaned_data = cleaned_data.drop(
                        index=issue.affected_rows, errors="ignore"
                    )
                    logger.info(f"Removed {len(issue.affected_rows)} outlier records")

        logger.info(
            f"Data cleaning completed. Rows: {len(data)} -> {len(cleaned_data)}"
        )
        return cleaned_data
