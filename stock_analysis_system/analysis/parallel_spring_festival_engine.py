    ...
    def start_cluster(self) -> Client:
        ...
        try:
            if self.config.enable_distributed and self.config.scheduler_address:
                ...
                logger.info(f"Connecting to distributed Dask cluster at {self.config.scheduler_address}")
                self.client = Client(self.config.scheduler_address)
            else:
                # Create local cluster
                logger.info(f"Starting local Dask cluster with {self.config.n_workers} workers")
    ...
    def get_cluster_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_cores": sum(w.get("nthreads", 0) for w in info.get("workers", {}).values()),
                "total_memory": sum(w.get("memory_limit", 0) for w in info.get("workers", {}).values()),
...
class ParallelSpringFestivalEngine(SpringFestivalAlignmentEngine):
    ...
    def __init__(self, window_days: int = None, config: ParallelProcessingConfig = None):
    ...
    def analyze_multiple_stocks_parallel(
        ...
        try:
            with self.resource_manager as rm:
                if rm.client:
                    ...
                    logger.warning("Dask cluster not available, falling back to thread-based processing")
        ...
        logger.info(f"Success rate: {result.success_rate:.1%} ({len(successful_analyses)}/{len(stock_data_dict)})")
    ...
    def _analyze_with_dask(
        ...
        chunks = [symbols[i:i + self.config.chunk_size] 
        ...
        try:
            with self.resource_manager as rm:
                ...
                for batch_data in self._stream_data_batches(data_source, batch_size):
                    batch_result = self.analyze_multiple_stocks_parallel(batch_data, years)
                    
                    successful_analyses.update(batch_result.successful_analyses)
                    failed_analyses.update(batch_result.failed_analyses)
                    total_processed += len(batch_data)
                    
                    logger.info(f"Processed batch: {len(batch_data)} stocks, "
    ...
    def optimize_for_memory_usage(
        ...
        for symbol, data in stock_data_dict.items():
            ...
            if 'volume' in optimized_df.columns:
                optimized_df['volume'] = pd.to_numeric(optimized_df['volume'], downcast='integer')
        ...
        # Check memory usage
        total_memory = sum(df.memory_usage(deep=True).sum() for df in optimized_data.values())
        memory_gb = total_memory / (1024**3)
        
        logger.info(f"Optimized data memory usage: {memory_gb:.2f}GB")
        
        if memory_gb > max_memory_gb:
            logger.warning(f"Memory usage ({memory_gb:.2f}GB) exceeds limit ({max_memory_gb}GB)")
    ...
    def get_processing_recommendations(
        self, 
        total_stocks: int,
        avg_data_points_per_stock: int = 2000
    ) -> Dict[str, Any]:
        """Get recommendations for optimal processing configuration."""
        
        # Estimate memory requirements
        estimated_memory_per_stock = avg_data_points_per_stock * 8 * 10  # Rough estimate in bytes
        total_estimated_memory_gb = (total_stocks * estimated_memory_per_stock) / (1024**3)
        
        # Recommend chunk size based on memory
        if total_estimated_memory_gb > 8:
            recommended_chunk_size = max(50, min(200, int(8 * 1024**3 / estimated_memory_per_stock)))
        else:
...
def create_dask_dataframe_from_stock_data(stock_data_dict: Dict[str, pd.DataFrame]) -> dd.DataFrame:
    ...
    dask_df = dd.from_pandas(combined_df, npartitions=max(1, len(stock_data_dict) // 100))
