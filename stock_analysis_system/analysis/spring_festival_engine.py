...
class ChineseCalendar:
    ...
    @classmethod
    def is_spring_festival_period(cls, check_date: date, window_days: int = 15) -> Tuple[bool, Optional[date]]:
...
class SpringFestivalAlignmentEngine:
    ...
    def create_spring_festival_windows(self, years: List[int]) -> List[SpringFestivalWindow]:
    ...
    def align_to_spring_festival(self, stock_data: pd.DataFrame, years: List[int] = None) -> AlignedTimeSeries:
        """Align stock data to Spring Festival dates."""
        logger.info(f"Aligning stock data to Spring Festival for symbol: {stock_data['stock_code'].iloc[0] if not stock_data.empty else 'unknown'}")
        ...
        if len(years) < self.min_years:
            raise ValueError(f"Insufficient years for analysis. Need at least {self.min_years}, got {len(years)}")
        ...
        for window in windows:
            ...
            if window_data.empty:
                logger.warning(f"No data found for {symbol} in {window.year} Spring Festival window")
                continue
            ...
            for _, row in window_data.iterrows():
                ...
                normalized_price = (row['close_price'] - baseline_price) / baseline_price * 100
        ...
        logger.info(f"Aligned {len(aligned_points)} data points across {len(years)} years")
        return aligned_series
    
    def identify_seasonal_patterns(self, aligned_data: AlignedTimeSeries) -> SeasonalPattern:
        ...
        if before_sf_stats:
            before_returns = [stats['mean_return'] for stats in before_sf_stats.values()]
        ...
        all_returns = [(day, stats['mean_return']) for day, stats in daily_stats.items()]
    ...
    def _calculate_pattern_strength(self, aligned_data: AlignedTimeSeries) -> float:
        ...
        for year in aligned_data.years:
            ...
            for i in range(1, len(year_data)):
                if year_data[i].price > 0 and year_data[i-1].price > 0:
                    ret = (year_data[i].price - year_data[i-1].price) / year_data[i-1].price
                    returns.append(ret)
        ...
        for i in range(len(years_list)):
            for j in range(i + 1, len(years_list)):
                ...
                if min_len >= 5:
                    corr = np.corrcoef(year1_returns[:min_len], year2_returns[:min_len])[0, 1]
    ...
    def _calculate_consistency_score(self, daily_stats: Dict[int, Dict[str, float]]) -> float:
    ...
    def _calculate_confidence_level(self, aligned_data: AlignedTimeSeries, daily_stats: Dict[int, Dict[str, float]]) -> float:
        ...
        significant_days = sum(1 for stats in daily_stats.values() if stats['count'] >= 3)
        total_days = len(daily_stats)
        significance_factor = significant_days / total_days if total_days > 0 else 0.0
        
        # Combined confidence
        confidence = (years_factor * 0.4 + density_factor * 0.3 + significance_factor * 0.3)
        
        return min(confidence, 1.0)
    
    def get_current_position(self, symbol: str, current_date: date = None) -> Dict[str, Any]:
    ...
    def generate_trading_signals(self, pattern: SeasonalPattern, current_position: Dict[str, Any]) -> Dict[str, Any]:
