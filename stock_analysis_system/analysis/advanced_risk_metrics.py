...
class AdvancedRiskMetricsCalculator:
    ...
    async def calculate_advanced_metrics(self,
                                       ...
                                       spring_festival_engine = None) -> AdvancedRiskMetrics:
        """
        ...
        try:
            ...
            sharpe_ratio, sharpe_ci = await self._calculate_enhanced_sharpe_ratio(returns)
            sortino_ratio, sortino_ci = await self._calculate_enhanced_sortino_ratio(returns)
            calmar_ratio, calmar_ci = await self._calculate_enhanced_calmar_ratio(price_data, returns)
            
            # Calculate market risk metrics
            beta, beta_ci, alpha, tracking_error, info_ratio = await self._calculate_market_risk_metrics(
    ...
    async def _calculate_enhanced_sharpe_ratio(self, returns: pd.Series) -> Tuple[float, Optional[Tuple[float, float]]]:
        ...
        for _ in range(self.bootstrap_iterations):
            ...
            if bootstrap_sample.std() > 0:
                bootstrap_sharpe = (bootstrap_excess.mean() / bootstrap_sample.std()) * np.sqrt(252)
    ...
    async def _calculate_enhanced_sortino_ratio(self, returns: pd.Series) -> Tuple[float, Optional[Tuple[float, float]]]:
        ...
        for _ in range(self.bootstrap_iterations):
            ...
            if len(bootstrap_downside) > 0 and bootstrap_downside.std() > 0:
                bootstrap_sortino = (bootstrap_excess.mean() / bootstrap_downside.std()) * np.sqrt(252)
    ...
    async def _calculate_enhanced_calmar_ratio(self, price_data: pd.DataFrame, 
                                             returns: pd.Series) -> Tuple[float, Optional[Tuple[float, float]]]:
    ...
    async def _calculate_market_risk_metrics(self, returns: pd.Series, 
                                           benchmark_data: Optional[pd.DataFrame]) -> Tuple[Optional[float], Optional[Tuple[float, float]], Optional[float], Optional[float], Optional[float]]:
        ...
        try:
            ...
            alpha = stock_mean - (self.risk_free_rate + beta * (bench_mean - self.risk_free_rate))
            ...
            beta_bootstrap = []
            for _ in range(self.bootstrap_iterations):
                bootstrap_indices = np.random.choice(len(aligned_data), size=len(aligned_data), replace=True)
            ...
            if beta_bootstrap:
                ...
                beta_ci_upper = np.percentile(beta_bootstrap, (1 - alpha_level/2) * 100)
                beta_confidence_interval = (beta_ci_lower, beta_ci_upper)
            else:
                beta_confidence_interval = None
            
            return beta, beta_confidence_interval, alpha, tracking_error, information_ratio
            
        except Exception as e:
            logger.error(f"Error calculating market risk metrics: {e}")
            return None, None, None, None, None
    
    async def _calculate_liquidity_risk_metrics(self, price_data: pd.DataFrame, 
                                              volume_data: Optional[pd.DataFrame]) -> Dict[str, Union[float, str]]:
        ...
        try:
            ...
            merged_data['amihud'] = merged_data['price_change'] / merged_data['dollar_volume'].replace(0, np.nan)
            amihud_illiquidity = merged_data['amihud'].median()  # Use median to avoid outliers
            ...
            amihud_score = min(100, amihud_illiquidity * 1e6) if not np.isnan(amihud_illiquidity) else 50
            spread_score = min(100, avg_spread * 500)
            zero_volume_score = zero_volume_ratio * 100
            impact_score = min(100, market_impact_score * 1000) if not np.isnan(market_impact_score) else 50
    ...
    async def _calculate_price_based_liquidity_metrics(self, price_data: pd.DataFrame) -> Dict[str, Union[float, str]]:
        """Calculate liquidity metrics using only price data."""
        
        # Use high-low spread as primary liquidity proxy
        price_data = price_data.copy()
        price_data['spread_proxy'] = (price_data['high'] - price_data['low']) / price_data['close']
    ...
    async def _calculate_seasonal_risk_metrics(self, price_data: pd.DataFrame, 
                                             returns: pd.Series,
                                             spring_festival_engine) -> Dict[str, Union[float, str, dict]]:
        ...
        try:
            ...
            if hasattr(returns_with_date.index, 'month'):
                for month in range(1, 13):
                    month_returns = returns_with_date[returns_with_date.index.month == month]
                    if len(month_returns) > 5:  # Need minimum data
                        monthly_volatility[f"month_{month}"] = month_returns.std() * np.sqrt(252)
                    else:
                        monthly_volatility[f"month_{month}"] = returns.std() * np.sqrt(252)
            ...
            if spring_festival_engine is not None:
                try:
                    ...
                    nearest_sf_date = min(sf_dates, key=lambda x: abs((x - current_date).days))
                    ...
                    if len(price_data) > 500:  # Need sufficient historical data
                        sf_volatility_analysis = await self._analyze_spring_festival_volatility(
                            price_data, returns, sf_dates
                        )
                        
                        # Adjust seasonal risk score based on historical patterns
                        if sf_volatility_analysis['avg_sf_volatility'] > sf_volatility_analysis['normal_volatility'] * 1.3:
                            seasonal_risk_score = min(95.0, seasonal_risk_score * 1.2)
                            spring_festival_adjustment = min(2.0, spring_festival_adjustment * 1.1)
    ...
    async def _get_spring_festival_dates(self, start_year: int, end_year: int) -> List[datetime]:
    ...
    async def _analyze_spring_festival_volatility(self, price_data: pd.DataFrame, 
                                                returns: pd.Series,
                                                sf_dates: List[datetime]) -> Dict[str, float]:
        ...
        try:
            ...
            for sf_date in sf_dates:
                ...
                normal_mask1 = (returns.index >= normal_start1) & (returns.index <= normal_end1)
                normal_mask2 = (returns.index >= normal_start2) & (returns.index <= normal_end2)
            ...
            sf_volatility = np.std(sf_period_returns) * np.sqrt(252) if sf_period_returns else 0
            normal_volatility = np.std(normal_period_returns) * np.sqrt(252) if normal_period_returns else 0
            
            return {
                'avg_sf_volatility': sf_volatility,
                'normal_volatility': normal_volatility,
                'volatility_ratio': sf_volatility / normal_volatility if normal_volatility > 0 else 1.0
            }
    ...
    async def _calculate_omega_ratio(self, returns: pd.Series, threshold: float = 0.0) -> Optional[float]:
...
async def calculate_comprehensive_risk_profile(price_data: pd.DataFrame,
                                             benchmark_data: Optional[pd.DataFrame] = None,
                                             volume_data: Optional[pd.DataFrame] = None,
                                             spring_festival_engine = None,
                                             risk_engine = None) -> Dict[str, any]:
    """
    ...
    comprehensive_profile = {
        ...
        'summary': {
            'overall_risk_score': _calculate_overall_risk_score(basic_metrics, advanced_metrics),
            'risk_level': _determine_overall_risk_level(basic_metrics, advanced_metrics),
            'key_risk_factors': _identify_key_risk_factors(basic_metrics, advanced_metrics),
...
def _calculate_overall_risk_score(basic_metrics, advanced_metrics) -> float:
    ...
    try:
        ...
        var_score = basic_metrics.var_results.get('historical', type('obj', (object,), {'var_95': 0.05})).var_95 * 1000  # Scale VaR
        volatility_score = basic_metrics.volatility_results.get('historical', type('obj', (object,), {'annualized_volatility': 0.3})).annualized_volatility * 100
...
def _identify_key_risk_factors(basic_metrics, advanced_metrics) -> List[str]:
    ...
    try:
        ...
        if historical_vol and historical_vol.annualized_volatility > 0.4:  # 40% annual volatility
...
def _generate_risk_recommendations(basic_metrics, advanced_metrics) -> List[str]:
    ...
    try:
        ...
        if advanced_metrics.liquidity_risk_score > 60:
            recommendations.append("Monitor liquidity conditions and avoid large position sizes")
    ...
    return recommendations if recommendations else ["Maintain standard risk management practices"]