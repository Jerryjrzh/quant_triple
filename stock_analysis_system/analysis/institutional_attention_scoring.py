...
class StockAttentionProfile:
    ...
    coordination_clusters: List[List[str]] = field(
        default_factory=list
    )  # Institution ID clusters
...
class AttentionScoreCalculator:
    ...
    def calculate_attention_score(
        ...
        relevant_activities = [a for a in activities if a.activity_date >= cutoff_date]
        ...
        recency_score = self._calculate_recency_score(
            relevant_activities, reference_date
        )
        ...
        avg_activity_size = (
            total_amount / len(relevant_activities) if total_amount else None
        )
    ...
    def _create_zero_score(
        self, stock_code: str, institution: InstitutionalInvestor, reference_date: date
    ) -> AttentionScore:
    ...
    def _calculate_recency_score(
        self, activities: List[InstitutionalActivity], reference_date: date
    ) -> float:
    ...
    def _calculate_volume_score(self, activities: List[InstitutionalActivity]) -> float:
        ...
        volumes = [
            a.volume for a in activities if a.volume is not None and a.volume > 0
        ]
        amounts = [
            a.amount for a in activities if a.amount is not None and a.amount > 0
        ]
    ...
    def _calculate_frequency_score(
        ...
        if len(weekly_counts) > 1:
            ...
            consistency = (
                1.0 - min(std_count / mean_count, 1.0) if mean_count > 0 else 0.0
            )
        else:
            consistency = 1.0
    ...
    def _detect_behavior_pattern(
        ...
        buy_activities = [
            a for a in activities if self._is_buy_activity(a.activity_type)
        ]
        sell_activities = [
            a for a in activities if self._is_sell_activity(a.activity_type)
        ]
    ...
    def _classify_activity_intensity(
        self, recent_activities: List[InstitutionalActivity]
    ) -> ActivityIntensity:
    ...
    def _calculate_trend_consistency(
        ...
        sell_count = sum(
            1 for a in activities if self._is_sell_activity(a.activity_type)
        )
    ...
    def _calculate_activity_regularity(
        self, activities: List[InstitutionalActivity]
    ) -> float:
...
class InstitutionalAttentionScoringSystem:
    ...
    async def calculate_stock_attention_profile(
        self,
        stock_code: str,
        start_date: date,
        end_date: date,
        ...
        try:
            ...
            if self.graph_analytics:
                try:
                    ...
                    for inst_id in institutions.keys():
                        relationships = (
                            self.graph_analytics.get_institution_relationships(inst_id)
                        )
                        if relationships:
                            avg_strength = np.mean(
                                [r.strength_score for r in relationships]
                            )
                            coordination_scores[inst_id] = (
                                avg_strength * 100
                            )  # Scale to 0-100
                        else:
                    ...
                    coordination_scores = {
                        inst_id: 0.0 for inst_id in institutions.keys()
                    }
            else:
            ...
            profile = self._create_stock_profile(
                stock_code, institution_scores, activities
            )
            ...
            logger.info(
                f"Calculated attention profile for {stock_code}: "
    ...
    def _create_stock_profile(
        ...
        total_attention_score = np.mean(
            [score.overall_score for score in institution_scores]
        )
        institutional_count = len(institution_scores)
        active_institutional_count = len(
            [s for s in institution_scores if s.recent_activities > 0]
        )

        # Analyze patterns
        pattern_counts = Counter(
            [score.behavior_pattern for score in institution_scores]
        )
        dominant_patterns = pattern_counts.most_common(5)

        # Attention level distribution
        attention_distribution = Counter(
            [score.attention_level for score in institution_scores]
        )

        # Institution type analysis
        type_distribution = Counter(
            [score.institution.institution_type for score in institution_scores]
        )
        ...
        for inst_type in type_distribution.keys():
            type_scores = [
                s.overall_score
                for s in institution_scores
                if s.institution.institution_type == inst_type
            ]
            type_attention_scores[inst_type] = (
                np.mean(type_scores) if type_scores else 0.0
            )
        ...
        if total_activities > recent_activities:
            historical_rate = (total_activities - recent_activities) / max(
                1, total_activities - recent_activities
            )
        ...
        coordination_scores = [
            score.coordination_strength for score in institution_scores
        ]
        coordination_score = (
        ...
        high_coord_institutions = [
            score.institution.institution_id
            for score in institution_scores
            if score.coordination_strength > 0.5
        ]
        coordination_clusters = (
            [high_coord_institutions] if len(high_coord_institutions) > 1 else []
        )
    ...
    async def screen_stocks_by_attention(
        self,
                ...
                try:
                    ...
                    if self._meets_screening_criteria(profile, criteria):
                        ...
                        result = {
                            ...
                            "dominant_pattern": (
                                profile.dominant_patterns[0][0].value
                            ...
                            "screening_reasons": self._get_screening_reasons(
                                profile, criteria
                            ),
                        }
    ...
    def _meets_screening_criteria(
        self, profile: StockAttentionProfile, criteria: Dict[str, Any]
    ) -> bool:
    ...
    def _get_screening_reasons(
        ...
        if profile.active_institutional_count > 0:
            reasons.append(
                f"Recent activity: {profile.active_institutional_count} active institutions"
            )
    ...
    def _generate_alert_message(self, profile: StockAttentionProfile) -> str:
        ...
        message += (
            f"{inst_count} institutions tracked, {active_count} recently active. "
        )
    ...
    def get_institution_attention_summary(self, institution_id: str) -> Dict[str, Any]:
        ...
        institution_scores = [
            score
            for (inst_id, stock_code), score in self.institution_scores.items()
        ...
        high_attention_stocks = len(
            [s for s in institution_scores if s.overall_score > 70]
        )
        ...
        top_stocks = sorted(
            institution_scores, key=lambda s: s.overall_score, reverse=True
        )[:10]

        return {
            "institution_id": institution_id,
            "institution_name": institution_scores[0].institution.name,
            "institution_type": institution_scores[
                0
