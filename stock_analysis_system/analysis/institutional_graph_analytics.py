...
class InstitutionalRelationship:
    ...
    coordinated_activities: List[Tuple[InstitutionalActivity, InstitutionalActivity]] = field(default_factory=list)
...
class InstitutionalGraphAnalytics:
    ...
    def __init__(self, data_collector: InstitutionalDataCollector):
        ...
        # Analysis parameters
        self.coordination_time_window = timedelta(days=3)  # Time window for coordinated activity
        self.min_relationship_strength = 0.3  # Minimum strength for relationship inclusion
    ...
    async def build_institutional_network(self, 
                                        stock_codes: List[str], 
                                        start_date: date, 
                                        end_date: date,
                                        rebuild: bool = False) -> nx.Graph:
        """
        ...
        try:
            ...
            for stock_code, data in activity_data.items():
                if stock_code in self.data_collector.activity_timeline:
                    all_activities.extend(self.data_collector.activity_timeline[stock_code])
    ...
    async def _detect_relationships(self, activities: List[InstitutionalActivity]) -> None:
        ...
        for i, inst_a_id in enumerate(institution_ids):
            for inst_b_id in institution_ids[i+1:]:
                ...
                if relationships:
                    best_relationship = max(relationships, key=lambda r: r.strength_score)
                    if best_relationship.strength_score >= self.min_relationship_strength:
    ...
    async def _detect_coordinated_trading(self, 
                                        ...
                                        activities_b: List[InstitutionalActivity]) -> Optional[InstitutionalRelationship]:
        ...
        for activity_a in activities_a:
            for activity_b in activities_b:
                ...
                time_diff = abs((activity_a.activity_date - activity_b.activity_date).days)
                if time_diff > self.coordination_time_window.days:
                    continue
                
                # Check for coordination patterns
                coordination_score = self._calculate_coordination_score(activity_a, activity_b)
                
                if coordination_score > 0.5:  # Threshold for coordination
                    coordinated_pairs.append((activity_a, activity_b, coordination_score))
    ...
    def _calculate_coordination_score(self, 
                                    activity_a: InstitutionalActivity,
                                    activity_b: InstitutionalActivity) -> float:
    ...
    def _are_activities_coordinated(self, type_a: ActivityType, type_b: ActivityType) -> bool:
        ...
        return (type_a, type_b) in coordinated_patterns or (type_b, type_a) in coordinated_patterns
    
    def _detect_same_stock_activity(self,
                                  inst_a: InstitutionalInvestor,
                                  inst_b: InstitutionalInvestor,
                                  activities_a: List[InstitutionalActivity],
                                  activities_b: List[InstitutionalActivity]) -> Optional[InstitutionalRelationship]:
    ...
    def _detect_temporal_correlation(self,
                                   inst_a: InstitutionalInvestor,
                                   inst_b: InstitutionalInvestor,
                                   activities_a: List[InstitutionalActivity],
                                   activities_b: List[InstitutionalActivity]) -> Optional[InstitutionalRelationship]:
    ...
    def _detect_fund_family_relationship(self,
                                       inst_a: InstitutionalInvestor,
                                       inst_b: InstitutionalInvestor) -> Optional[InstitutionalRelationship]:
        """Detect fund family relationships based on name similarity"""
        
        # Only apply to funds
        if (inst_a.institution_type not in [InstitutionType.MUTUAL_FUND] or
            inst_b.institution_type not in [InstitutionType.MUTUAL_FUND]):
            return None
            ...
            if relationship.coordinated_activities:
                ...
                if buy_sell_balance != 0:
                    if buy_sell_balance > 0:
                        ...
                        self.directed_graph.add_edge(inst_a_id, inst_b_id, **self.graph[inst_a_id][inst_b_id])
                    else:
                        # inst_b tends to lead
                        self.directed_graph.add_edge(inst_b_id, inst_a_id, **self.graph[inst_a_id][inst_b_id])
    
    def _analyze_coordination_direction(self, relationship: InstitutionalRelationship) -> float:
        ...
        return direction_score / len(relationship.coordinated_activities) if relationship.coordinated_activities else 0.0
    ...
    async def detect_coordinated_patterns(self, 
                                        min_institutions: int = 3,
                                        min_correlation: float = 0.7) -> List[CoordinatedActivityPattern]:
        """
        ...
        try:
            ...
            for stock_code, activities in stock_activities.items():
                ...
                time_windows = self._create_time_windows(activities, self.coordination_time_window)
                ...
                for window_start, window_activities in time_windows.items():
                    ...
                    for activity in window_activities:
                        inst_activities[activity.institution.institution_id].append(activity)
    ...
    def _create_time_windows(self, 
                           ...
                           window_size: timedelta) -> Dict[datetime, List[InstitutionalActivity]]:
    ...
    def _analyze_coordination_pattern(self,
                                    stock_code: str,
                                    window_start: datetime,
                                    inst_activities: Dict[str, List[InstitutionalActivity]],
                                    min_correlation: float) -> Optional[CoordinatedActivityPattern]:
        """Analyze a time window for coordination patterns"""
        
        institutions = [self.institutions[inst_id] for inst_id in inst_activities.keys()]
        ...
        for i, inst_a_id in enumerate(inst_ids):
            for inst_b_id in inst_ids[i+1:]:
                ...
                correlation = self._calculate_activity_correlation(activities_a, activities_b)
    ...
    def _calculate_activity_correlation(self,
                                      activities_a: List[InstitutionalActivity],
                                      activities_b: List[InstitutionalActivity]) -> float:
    ...
    def _calculate_volume_correlation(self, inst_activities: Dict[str, List[InstitutionalActivity]]) -> float:
    ...
    def create_network_visualization(self, 
                                   ...
                                   color_by: str = "institution_type") -> go.Figure:
        """
        ...
        for node in self.graph.nodes():
            ...
            if institution:
                ...
                node_info.append(f"Name: {institution.name}<br>"
                               f"Type: {institution.institution_type.value}<br>"
                               f"Confidence: {institution.confidence_score:.2f}")
                
                # Color by institution type
                node_colors.append(type_colors.get(institution.institution_type.value, '#7f7f7f'))
                
                # Size by centrality metric
                if self.network_metrics and node_size_metric in ['degree_centrality', 'betweenness_centrality', 'closeness_centrality']:
                    centrality_dict = getattr(self.network_metrics, node_size_metric, {})
        ...
        for edge in self.graph.edges():
            ...
            if relationship:
                edge_info.append(f"Type: {relationship.relationship_type.value}<br>"
                               f"Strength: {relationship.strength_score:.2f}<br>"
                               f"Common Stocks: {len(relationship.common_stocks)}")
                edge_widths.append(1 + relationship.strength_score * 4)  # Scale between 1-5
            else:
    ...
    def get_institution_relationships(self, institution_id: str) -> List[InstitutionalRelationship]:
                ...
                "by_degree_centrality": [
                    {
                        ...
                        "name": self.institutions[inst_id].name if inst_id in self.institutions else inst_id,
