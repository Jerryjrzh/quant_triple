...
class InstitutionClassifier:
    ...
    def _load_classification_rules(self) -> Dict[InstitutionType, List[Dict]]:
        ...
        return {
            InstitutionType.MUTUAL_FUND: [
                ...
                {"pattern": r".*asset.*management.*", "confidence": 0.7, "case_sensitive": False}
            ],
            ...
            InstitutionType.INSURANCE: [
                ...
                {"pattern": r".*insurance.*", "confidence": 0.8, "case_sensitive": False},
            ...
            InstitutionType.SECURITIES_FIRM: [
                ...
                {"pattern": r".*securities.*", "confidence": 0.8, "case_sensitive": False},
    ...
    def classify_institution(self, name: str, additional_info: Optional[Dict] = None) -> Tuple[InstitutionType, float]:
        """
    ...
    def create_institution(self, name: str, additional_info: Optional[Dict] = None) -> InstitutionalInvestor:
...
class DataCollector(ABC):
    ...
    @abstractmethod
    async def collect_data(self, stock_code: str, start_date: date, end_date: date) -> List[Any]:
...
class DragonTigerCollector(DataCollector):
    ...
    async def collect_data(self, stock_code: str, start_date: date, end_date: date) -> List[DragonTigerRecord]:
        """
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                if np.random.random() < 0.1:  # 10% chance of Dragon-Tiger activity
                    ...
                    for i in range(np.random.randint(1, 6)):  # 1-5 buy records
                        ...
                        amount = np.random.uniform(10000000, 500000000)  # 10M to 500M yuan
            ...
            logger.info(f"Collected {len(records)} Dragon-Tiger records for {stock_code}")
            return records
...
class ShareholderCollector(DataCollector):
    ...
    async def collect_data(self, stock_code: str, start_date: date, end_date: date) -> List[ShareholderRecord]:
        """
        ...
        try:
                ...
                if current_date.month in [3, 6, 9, 12] and current_date.day >= 28:
                    ...
                    for rank in range(1, 11):  # Top 10 shareholders
                        if rank == 1:
                            ...
                            shares = int(remaining_shares * np.random.uniform(0.05, 0.15))
                            shareholder_name = self._generate_mock_institutional_name()
                        else:
                            # Smaller institutional shareholders
                            shares = int(remaining_shares * np.random.uniform(0.01, 0.08))
                            shareholder_name = self._generate_mock_institutional_name()
                        ...
                        # Classify institution
                        institution = self.classifier.create_institution(shareholder_name)
                        ...
                        record = ShareholderRecord(
                            ...
                            institution_confidence=institution.confidence_score,
                            rank=rank,
                            shareholder_type="institutional" if rank > 1 else "controlling"
                        )
                        records.append(record)
            ...
            logger.info(f"Collected {len(records)} shareholder records for {stock_code}")
            return records
...
class BlockTradeCollector(DataCollector):
    ...
    async def collect_data(self, stock_code: str, start_date: date, end_date: date) -> List[BlockTradeRecord]:
        """
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                if np.random.random() < 0.05:  # 5% chance of block trade
                    ...
                    for _ in range(np.random.randint(1, 4)):
                        volume = np.random.randint(100000, 10000000)  # 100K to 10M shares
                        ...
                        buyer_institution = self.classifier.create_institution(buyer_seat)
                        seller_institution = self.classifier.create_institution(seller_seat)
                        
                        # Calculate discount/premium
                        market_price = price * np.random.uniform(0.98, 1.02)  # Market price
            ...
            logger.info(f"Collected {len(records)} block trade records for {stock_code}")
            return records
...
class InstitutionalDataCollector:
    ...
    async def collect_all_data(self, 
                             ...
                             start_date: date, 
                             end_date: date) -> Dict[str, Dict[str, List]]:
        """
        ...
        try:
            ...
            async with self.dragon_tiger_collector, \
                ...
                for stock_code in stock_codes:
                    ...
                    activities = self._consolidate_activities(
                        stock_code, dragon_tiger_data, shareholder_data, block_trade_data
                    )
                    self.activity_timeline[stock_code] = activities
                    
                    logger.info(f"Collected {len(dragon_tiger_data)} Dragon-Tiger, "
    ...
    def _consolidate_activities(self, 
                              stock_code: str,
                              dragon_tiger_data: List[DragonTigerRecord],
                              shareholder_data: List[ShareholderRecord],
                              block_trade_data: List[BlockTradeRecord]) -> List[InstitutionalActivity]:
    ...
    def get_institution_activity_timeline(self, 
                                        stock_code: str, 
                                        institution_type: Optional[InstitutionType] = None) -> List[InstitutionalActivity]:
        ...
        if institution_type:
            activities = [a for a in activities if a.institution.institution_type == institution_type]
    ...
    def get_institution_summary(self, stock_code: str) -> Dict[str, Any]:
        ...
        for activity in activities:
            ...
            if activity.amount:
                type_amounts[inst_type] = type_amounts.get(inst_type, 0) + activity.amount
        ...
        if recent_date:
            ...
            recent_activities = [a for a in activities if a.activity_date >= cutoff_date]
        else:
            recent_activities = []
        ...
        return {
            ...
            'date_range': {
                'start': min(a.activity_date for a in activities) if activities else None,
