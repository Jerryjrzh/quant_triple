...
class SpringFestivalChartEngine:
    ...
    def create_overlay_chart(
        ...
        for i, year in enumerate(years_to_show):
            ...
            hover_text = [
                ...
                for date, rel_day, price, norm_price in zip(
                    dates, relative_days, actual_prices, normalized_prices
                )
            ]
    ...
    def create_pattern_summary_chart(
        self, patterns: Dict[str, SeasonalPattern], title: str = "春节模式对比分析"
    ) -> go.Figure:
        ...
        fig.update_layout(
            title=dict(
                text=title, x=0.5, font=dict(size=18, color=self.config.text_color)
            ),
            plot_bgcolor=self.config.background_color,
            paper_bgcolor=self.config.background_color,
            font=dict(color=self.config.text_color),
    ...
    def create_cluster_visualization(
        ...
        if len(features) < n_clusters:
            raise ValueError(
                f"Not enough valid patterns ({len(features)}) for {n_clusters} clusters"
            )
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_symbols = [
                symbols[i] for i in range(len(symbols)) if cluster_mask[i]
            ]
        ...
        try:
            if format == "html":
                ...
                html_str = pio.to_html(
                    fig, include_plotlyjs=True, config={"displayModeBar": True}
                )
                ...
                img_bytes = pio.to_image(
                    fig, format=format, scale=self.config.export_scale, **kwargs
                )
    ...
    def create_interactive_dashboard(
        ...
        for i, (symbol, aligned_data) in enumerate(
            list(aligned_data_dict.items())[:5]
        ):  # Limit to 5 stocks
        ...
        total_returns = [
            patterns[symbol].average_return_before
            + patterns[symbol].average_return_after
            for symbol in symbols
        ]
        ...
        fig.update_layout(
            title=dict(
                text=title, x=0.5, font=dict(size=20, color=self.config.text_color)
            ),
            plot_bgcolor=self.config.background_color,
            paper_bgcolor=self.config.background_color,
            font=dict(color=self.config.text_color),
...
def create_sample_chart(symbol: str = "000001", years: List[int] = None) -> go.Figure:
    ...
    return chart_engine.create_overlay_chart(
        aligned_data, title=f"{symbol} 春节对齐分析示例"
    )
