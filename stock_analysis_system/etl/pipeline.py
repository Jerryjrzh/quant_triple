...
from stock_analysis_system.core.database import AsyncSessionLocal, get_db_session
...
class ETLPipeline:
    ...
    async def run_job(self, config: ETLJobConfig) -> ETLMetrics:
        ...
        self.metrics = ETLMetrics(
            start_time=datetime.now(), stage=ETLStage.EXTRACT, status=ETLStatus.RUNNING
        )
    ...
    async def _extract_data(self, config: ETLJobConfig) -> Optional[pd.DataFrame]:
        ...
        for symbol in config.symbols:
            try:
                ...
                failed_symbols.append(symbol)
                self.metrics.error_messages.append(
                    f"Extract failed for {symbol}: {str(e)}"
                )

        if failed_symbols:
            logger.warning(
                f"Failed to extract data for {len(failed_symbols)} symbols: {failed_symbols}"
            )
    ...
    async def _validate_data_quality(self, data: pd.DataFrame, job_name: str):
        ...
        try:
            ...
            return DataQualityReport(
                ...
                accuracy_score=0.5,
                recommendations=[
                    "Data quality validation failed - manual review recommended"
                ],
            )

    async def _transform_data(
        self, data: pd.DataFrame, config: ETLJobConfig
        ...
        try:
            ...
            missing_columns = [
                col for col in required_columns if col not in transformed_data.columns
            ]
            ...
            logger.info(
                f"Data transformation completed: {len(transformed_data)} records"
            )
        ...
        try:
            ...
            for i in range(0, len(data), batch_size):
                ...
                try:
                    ...
                    logger.debug(
                        f"Loaded batch {i//batch_size + 1}: {batch_result['loaded']} records"
                    )
            ...
            logger.info(
                f"Data loading completed: {loaded_count} loaded, {failed_count} failed"
            )
    ...
    async def _load_batch(
        self, batch: pd.DataFrame, config: ETLJobConfig
    ) -> Dict[str, int]:
