...
from stock_analysis_system.etl.pipeline import ETLJobConfig, ETLStatus, get_etl_manager
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.daily_data_ingestion",
)
...
async def _run_daily_ingestion(
    symbols: Optional[List[str]], days_back: int
) -> Dict[str, Any]:
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.historical_backfill",
)
def historical_backfill(self, symbols: List[str], years: int = 5):
    """Historical data backfill task."""
    logger.info(
        f"Starting historical backfill for {len(symbols)} symbols, {years} years"
    )
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.data_quality_check",
)
...
async def _run_quality_check(symbols: List[str], days_back: int) -> Dict[str, Any]:
    ...
    for symbol in symbols:
        try:
            ...
            results["symbol_results"][symbol] = {"status": "error", "error": str(e)}

    # Calculate overall metrics
    checked_symbols = [
        s for s, r in results["symbol_results"].items() if r.get("status") == "checked"
    ]
    if checked_symbols:
    ...
    results["recommendations"] = list(
        set(results["recommendations"])
    )  # Remove duplicates

    logger.info(
        f"Quality check completed: {len(checked_symbols)} symbols, "
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.data_transformation",
)
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.generate_quality_report",
)
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _generate_quality_report(symbols, days_back)
            )
            return result
        finally:
            loop.close()
...
async def _generate_quality_report(
    ...
    report = {
        ...
        "summary": {
            ...
            "symbols_with_data": len(
                [
                    s
                    for s, r in quality_results["symbol_results"].items()
    ...
    logger.info(
        f"Quality report generated: {report['summary']['quality_grade']} grade, "
...
@celery_app.task(
    bind=True,
    base=CallbackTask,
    name="stock_analysis_system.etl.tasks.cleanup_old_data",
)
...
async def _cleanup_old_data(days_to_keep: int) -> Dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            logger.info(
                f"Cleaned up {records_to_delete} records older than {cutoff_date}"
            )
...
def schedule_daily_ingestion(
    symbols: Optional[List[str]] = None, eta: Optional[datetime] = None
):
    """Schedule daily data ingestion task."""
    return daily_data_ingestion.apply_async(
        args=[symbols], eta=eta, queue="data_ingestion"
    )


def schedule_historical_backfill(
    symbols: List[str], years: int = 5, eta: Optional[datetime] = None
):
    """Schedule historical backfill task."""
    return historical_backfill.apply_async(
        args=[symbols, years], eta=eta, queue="data_ingestion"
    )


def schedule_quality_check(
    symbols: List[str], days_back: int = 30, eta: Optional[datetime] = None
):
    """Schedule data quality check task."""
    return data_quality_check.apply_async(
        args=[symbols, days_back], eta=eta, queue="data_quality"
    )
