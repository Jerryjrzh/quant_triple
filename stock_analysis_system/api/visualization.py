...
async def create_multi_stock_chart(request: MultiStockChartRequest):
    ...
    try:
        if len(request.symbols) > 10:
            raise HTTPException(
                status_code=400, 
                detail="Maximum 10 stocks allowed for comparison"
            )
...
async def get_available_chart_types():
    ...
    return {
        "single_stock": [
            {"type": "overlay", "name": "春节对齐叠加图", "description": "多年数据叠加显示"},
            {"type": "pattern", "name": "模式分析图", "description": "显示季节性模式信息"}
        ],
        "multi_stock": [
            {"type": "comparison", "name": "对比分析图", "description": "多股票模式对比"},
            {"type": "cluster", "name": "聚类分析图", "description": "基于模式的聚类可视化"},
            {"type": "dashboard", "name": "综合仪表板", "description": "多维度分析仪表板"}
        ],
